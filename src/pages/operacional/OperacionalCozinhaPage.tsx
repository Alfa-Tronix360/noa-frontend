...
export default function OperacionalCozinhaPage() {
    // DEPOIS
    const { data: pedidosData, isLoading } = useQuery({
        queryKey: ['fluxo-cozinha'],
        queryFn: () => http.get<unknown, any>('/operacional/fluxo/cozinha'),
        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                ...
                <div className="rounded-xl border border-border bg-surface p-8 text-center text-sm text-muted-foreground">
                    Sem pedidos de cozinha neste momento.
                </div>
            )}
        </div>
    )
